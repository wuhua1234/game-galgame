...
namespace Flower{
    ...
    public class FlowerSystem : MonoBehaviour
    {
        ...
        private bool useCustomTextWrapping = true;
       ...
       public void SetupDialog(string resourcePath="DefaultDialogPrefab", bool applyRoleNameSupport=true, bool applyDefaultTextUpdateEvent=true, bool applyCustomTextWrapping=true){
            try{
                ...
                if(applyDefaultTextUpdateEvent){
                    try{
                        ...
                        dialogTextComponent = dialog.transform.Find("DialogPanel/DialogText").GetComponent<Text>();
                        dialogTextRect = dialog.transform.Find("DialogPanel/DialogText").GetComponent<RectTransform>();
                        dialogTextComponent.text = "";
                        
                        // 设置自定义分行
                        useCustomTextWrapping = applyCustomTextWrapping;
                        if(useCustomTextWrapping){
                            dialogTextComponent.horizontalOverflow = HorizontalWrapMode.Overflow;
                            dialogTextComponent.verticalOverflow = VerticalWrapMode.Overflow;
                        }
                        _defaultTextUpdateHandler = (object sender, TextUpdateEventArgs args)=>{
                            // 更新角色名称和对话内容
                            var roleNameText = dialog.transform.Find("RoleName/DialogText")?.GetComponent<Text>();
                            if (roleNameText != null){
                                // 处理两种格式的角色名：[角色名]:对话内容 或 角色名:对话内容
                                if (args.text.Contains(":"))
                                {
                                    var parts = args.text.Split(new[] { ':' }, 2);
                                    roleNameText.text = parts[0].Trim();
                                    var content = parts[1].Trim();
                                    
                                    if(useCustomTextWrapping){
                                        content = ApplyCustomTextWrapping(content);
                                    }
        ...
        public void InvokeSpecialCommand(string specialCmd){
            ...
            try{
                if(specialCmdWords.Count == 0)
                    return;
                
                string _cmd = specialCmdWords[0];
                
                // 检查是否是可能的角色名（只有汉字、字母或数字，没有空格或特殊字符）
                bool isPossibleRoleName = true;
                foreach(char c in _cmd){
                    if(!char.IsLetterOrDigit(c) && !char.IsPunctuation(c)){
                        isPossibleRoleName = false;
                        break;
                    }
                }
                
                // 如果是可能的角色名且没有对应的特殊命令，则不报错
                if(isPossibleRoleName && !specialCharFuncMap.ContainsKey(_cmd)){
                    // 这可能是一个角色名，不报错
                    return;
                }
        ...
        private IEnumerator CmdFunc_image_Task(List<string> _params)
        {
            ...
            try{
                ...
                try{
                    ...
                    // 尝试解析缩放比例参数（可选）
                    try{
                        scale = float.Parse(_params[6].ToString());
                    }catch(Exception){}
                }catch(Exception){
                }
            }catch(Exception e){
            ...
            try{
                ...
                // 应用用户指定的缩放比例
                sceneObj.transform.localScale = new Vector3(spScale * scale, spScale * scale, sceneObj.transform.localScale.z);
            }catch(Exception e){
        ...
        private SpecialCharType CheckSpecialChar(char _char)
        {
            ...
            {
                // 检查是否是角色名格式 [角色名]:
                // 注意：这里我们不能直接检查下一个字符，因为CheckSpecialChar每次只处理一个字符
                // 所以我们需要检查当前特殊命令后面是否应该作为角色名处理
                ...
                // 只有在确定不是角色名格式时才调用特殊命令
                // 注意：由于我们无法提前知道下一个字符，所以需要在SetupDialog中处理角色名
        ...
        #region Custom Text Wrapping
        private string ApplyCustomTextWrapping(string text){
            if(string.IsNullOrEmpty(text) || dialogTextComponent == null || dialogTextRect == null) return text;
            
            // 获取文本框的可用宽度
            float availableWidth = dialogTextRect.rect.width;
            ...
            for(int i = 0; i < text.Length; i++){
                ...
                dialogTextComponent.text = testLine;
                float lineWidth = dialogTextComponent.preferredWidth;
                
                // 如果超出宽度，则换行
                if(lineWidth > availableWidth){
                    // 移除最后一个字符（因为它是导致超宽的字符）
            ...
            for(int i = 0; i < lines.Count; i++){
                ...
                // 如果当前行以标点符号开头，且不是第一行，则尝试将标点符号移到上一行
                if(i > 0 && IsPunctuationAtStart(line)){
                    var prevLine = processedLines[processedLines.Count - 1];
                    var lastChar = prevLine[prevLine.Length - 1];
                    
                    // 如果上一行最后一个字符不是标点符号，可以移动
                    if(!IsPunctuation(lastChar)){
                        // 将标点符号移到上一行
                        processedLines[processedLines.Count - 1] = prevLine + line[0];
        ...
        // 检查字符串是否以标点符号开头
        private bool IsPunctuationAtStart(string text){
            if(string.IsNullOrEmpty(text)) return false;
            return IsPunctuation(text[0]);
        }
